...
public interface ADStatisticsRepository extends JpaRepository<Order, String> {
    ...
    @Query(value = """
                                ...
                                FROM `order` o
                                LEFT JOIN order_detail od ON o.id = od.id_order
                                WHERE o.order_status = 'HOAN_THANH'
                                    AND o.created_date BETWEEN :startDate AND :endDate
                                GROUP BY date
                                ORDER BY date ASC
            """, nativeQuery = true)
    ...
    @Query(value = "SELECT COALESCE(SUM(total_after_discount), 0) FROM `order` WHERE order_status = 'HOAN_THANH' AND created_date BETWEEN :start AND :end", nativeQuery = true)
