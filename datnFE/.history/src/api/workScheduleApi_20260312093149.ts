...
//Lấy lịch làm việc
export const getSchedules = async (params: ScheduleSearchRequest): Promise<WorkScheduleResponse[]> => {
...
export const getTodaySchedule = async (employeeId: string): Promise<ResponseObject<{id: string, workDate: string}>> => {
  // Lưu ý: Đường dẫn này phải khớp với @GetMapping ở Backend của Nhung
  // Ví dụ Backend: @GetMapping("/today/{employeeId}")
  const res = await axiosClient.get<ResponseObject<{id: string, workDate: string}>>(
    `${BASE_URL}/today/${employeeId}`
  );
  return res.data;
};
