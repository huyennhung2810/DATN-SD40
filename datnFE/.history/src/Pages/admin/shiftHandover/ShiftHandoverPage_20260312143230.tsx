  ...
  useEffect(() => {
    ...
    const fetchTodaySchedule = async () => {
      ...
      try {
        ...
        // Kiểm tra nếu là lỗi 404 từ Backend (nghĩa là không có lịch)
        if (error?.status === "NOT_FOUND" || error?.response?.status === 404) {
          // Chúng ta giữ im lặng vì đây là trường hợp nhân viên không có lịch
          setTodaySchedule(null);
        } else {
          // Chỉ log những lỗi thực sự nghiêm trọng (như lỗi 500 hoặc mất mạng)
          console.error("Lỗi hệ thống khi lấy lịch trực:", error);
        }
      } finally {
