...
const ShiftTemplatePage: React.FC = () => {
  ...
  // Xử lý mở modal Sửa
  const handleEdit = (record: ShiftTemplateRecord) => {
    ...
    formModal.setFieldsValue({
      ...
      timeRange: [
        ...
        dayjs(record.endTime, "HH: [record.name] xmm:ss"),
      ],
    });
  };
  ...
  const columns: TableColumnsType<ShiftTemplateRecord> = [
    {
      title: "STT",
    ...
    },
    {
      ...
      key: "status",
      align: "center",
      render: (status: string, record: ShiftTemplateRecord) => (
        <Popconfirm
          title="Thay đổi trạng thái"
          onConfirm={() => handleStatusChange(record.id)}
        >
          <Switch checked={status === "ACTIVE"} loading={isLoading} />
        </Popconfirm>
      ),
    },
    {
