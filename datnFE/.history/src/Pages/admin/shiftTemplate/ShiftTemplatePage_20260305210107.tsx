...
const ShiftTemplatePage: React.FC = () => {
  ...
  const onFinishModal = (values: any) => {
    ...
    if (editingId) {
      console.log("Cập nhật ca làm việc:", editingId, payload);
    } else {
      dispatch(shiftTemplateActions.createRequest(payload));
    }
      ...
      render: (record: any) => (
        <Tooltip title="Chỉnh sửa">
          <Button
            ...
            onClick={() => handleEdit(record)} // Gọi hàm handleEdit ở đây
          />
        </Tooltip>
      ),
    },
...
function dayjs(startTime: any, arg1: string) {
