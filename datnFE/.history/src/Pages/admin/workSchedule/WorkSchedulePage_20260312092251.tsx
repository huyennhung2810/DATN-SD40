...
const WorkSchedulePage: React.FC = () => {
  ...
  const fetchSchedules = async (): Promise<void> => {
    ...
    try {
      ...
      // 🟢 CHỐT CHẶN: API trả về ResponseObject, nên ta lấy res.data
      // Kiểm tra nếu là mảng mới set, không thì set mảng rỗng
      ...
      setSchedules([]); // Lỗi cũng set mảng rỗng để filter không chết
    } finally {
      setLoading(false);
    }
  };
  ...
  const displayedSchedules = useMemo(() => {
    // 🟢 PHÒNG THỦ: Nếu schedules chưa kịp về hoặc lỗi, trả về mảng rỗng ngay
    if (!Array.isArray(schedules)) return [];

    return schedules.filter((item) => {
      // 🟢 Lọc theo ID nhân viên (Chính xác nhất)
      const matchEmp =
