...
const WorkSchedulePage: React.FC = () => {
  ...
  const fetchEmployees = async (): Promise<void> => {
    try {
      ...
      const empList = responseData?.data?.data || responseData?.data || [];
  ...
  const fetchSchedules = async (): Promise<void> => {
    ...
    try {
      ...
      // res lúc này là ResponseObject<WorkScheduleResponse[]>
      const res = await workScheduleApi.getSchedules(params);

      // 🟢 Lấy mảng dữ liệu từ res.data
      // Thêm check Array.isArray để cực kỳ an toàn
      const dataArray = Array.isArray(res.data) ? res.data : [];
      setSchedules(dataArray);
    } catch (error: unknown) {
      console.error(error);
      setSchedules([]); // Lỗi thì trả về mảng rỗng để không bị crash filter
    } finally {
      setLoading(false);
    }
  };
