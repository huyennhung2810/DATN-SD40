...
const WorkScheduleModal: React.FC<Props> = ({
  ...
  const fetchData = async () => {
    try {
      ...
      // Map dữ liệu ca mẫu (Sử dụng Optional Chaining an toàn)
  ...
  const handleSubmit = async (values: AssignShiftFormValues): Promise<void> => {
    setLoading(true);
    try {
      ...
      if (initialData) {
        await workScheduleApi.deleteSchedule(initialData.id);
      ...
      message.error(apiErr || "Lỗi khi xử lý ca làm việc");
    } finally {
      setLoading(false);
    }
  };
