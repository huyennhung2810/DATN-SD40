...
const WorkScheduleModal: React.FC<Props> = ({
  ...
  const fetchData = async () => {
    try {
      ...
      // Bóc tách dữ liệu nhân viên (Hỗ trợ cả mảng trực tiếp hoặc bọc trong .data)
      const empList =
      ...
      // Bóc tách dữ liệu ca mẫu
      const shiftList =
        (shiftRes as any)?.data || (Array.isArray(shiftRes) ? shiftRes : []);
      setShifts(
        shiftList.map((s: ADShiftTemplateResponse) => ({
  ...
  useEffect(() => {
    if (isOpen) {
      fetchData();
      // 2. Điền dữ liệu vào form khi Sửa (Dùng ID trực tiếp, cực kỳ nhanh)
      if (initialData) {
        form.setFieldsValue({
  ...
  const handleSubmit = async (values: AssignShiftFormValues) => {
    ...
    try {
      ...
      // Hiển thị lỗi từ Backend (Ví dụ: "Lịch bị trùng...")
      const errorMsg =
  ...
  return (
    <Modal
      ...
      onCancel={onClose}
      footer={null}
      destroyOnClose // ✅ Đảm bảo form sạch sẽ mỗi lần mở
    >
