...
import { Table, Card, Tag, Typography, Tabs, Button, Drawer, Space, message, Spin, Row, Col } from "antd";
import { EyeOutlined, CheckCircleOutlined, CarOutlined, InboxOutlined, StopOutlined } from "@ant-design/icons";
...
const OrderPage: React.FC = () => {
    ...
    const handleAssignSerials = async (serialNumbers: string[]) => {
        try {
            await orderApi.assignSerials(selectedOrder.id, activeDetailForSerial.id, serialNumbers);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (price: number) => <Text type="danger" strong>{price?.toLocaleString("vi-VN")} đ</Text>,
        },
        {
            title: "Trạng thái",
        ...
        },
        {
            ...
            dataIndex: "orderType",
            key: "orderType",
            render: (type: string) => <Tag color={type === "ONLINE" ? "geekblue" : "magenta"}>{type}</Tag>,
        },
        {
            title: "Hành động",
            key: "action",
            render: (_: any, record: any) => (
                <Button type="primary" icon={<EyeOutlined />} onClick={() => handleViewDetail(record)}>Chi tiết</Button>
            ),
        },
    ];
    ...
    const detailColumns = [
        {
            title: "Sản phẩm",
            render: (record: any) => record.productDetail?.product?.name || "Máy ảnh/Ống kính",
        },
        {
            title: "Đơn giá",
            dataIndex: "unitPrice",
    ...
    return (
        <>
            <Card>
                <Title level={4}>Quản Lý Đơn Hàng (Online / Web)</Title>
            </Card>

            <Card style={{ marginTop: 16 }}>
                <Tabs activeKey={activeTab} onChange={setActiveTab} items={OrderStatuses} />
                <Table
                ...
                extra={
                    <Space>
                        {selectedOrder?.orderStatus === "PENDING" && (
                            <Button type="primary" icon={<CheckCircleOutlined />} onClick={() => handleUpdateStatus("CONFIRMED")}>
                                Xác nhận đơn
                            </Button>
                        )}
                        {selectedOrder?.orderStatus === "CONFIRMED" && (
                            <Button style={{ background: "#17A2B8", color: "#fff" }} icon={<InboxOutlined />} onClick={() => handleUpdateStatus("PACKAGING")}>
                                Chốt Đóng gói
                            </Button>
                        )}
                        {selectedOrder?.orderStatus === "PACKAGING" && (
                            <Button style={{ background: "#6F42C1", color: "#fff" }} icon={<CarOutlined />} onClick={() => handleUpdateStatus("SHIPPING")}>
                                Giao ĐVVC
                            </Button>
                        )}
                        {selectedOrder?.orderStatus === "SHIPPING" && (
                            <Button type="primary" style={{ background: "#28A745", color: "#fff" }} icon={<CheckCircleOutlined />} onClick={() => handleUpdateStatus("COMPLETED")}>
                                Giao Thành Công (Sinh Bảo hành)
                            </Button>
                        )}
                        {["PENDING", "CONFIRMED"].includes(selectedOrder?.orderStatus) && (
                            <Button danger icon={<StopOutlined />} onClick={() => handleUpdateStatus("CANCELED")}>Hủy đơn</Button>
                        )}
                    </Space>
                }
            >
                <Row gutter={[16, 16]}>
                    <Col span={12}>
                        <Card size="small" title="Thông tin người nhận">
                            <p><Text strong>Người nhận:</Text> {selectedOrder?.recipientName}</p>
                            <p><Text strong>Điện thoại:</Text> {selectedOrder?.recipientPhone}</p>
                            <p><Text strong>Ghi chú:</Text> {selectedOrder?.note || "..."}</p>
                            <p><Text strong>Trạng thái CĐH:</Text> <Tag color={getStatusColor(selectedOrder?.orderStatus)}>{selectedOrder?.orderStatus}</Tag></p>
                        </Card>
                    </Col>
                    <Col span={12}>
                        <Card size="small" title="Tổng quan thanh toán">
                            <p><Text strong>Tổng tiền hàng:</Text> {selectedOrder?.totalAmount?.toLocaleString("vi-VN")} đ</p>
                            <p><Text strong>Giảm giá:</Text> {selectedOrder?.discountAmount?.toLocaleString("vi-VN") || 0} đ</p>
                            <p><Text strong>Phải thanh toán:</Text> <Text type="danger" strong>{selectedOrder?.totalAfterDiscount?.toLocaleString("vi-VN")} đ</Text></p>
                        </Card>
                    </Col>
                </Row>

                <Title level={5} style={{ marginTop: 24 }}>Danh sách sản phẩm xuất kho</Title>
                <Table
