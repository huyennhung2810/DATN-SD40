...
import { Modal, Input, Button, Table, message, Typography, Space, Spin, Tag } from 'antd';
import { QrcodeOutlined, DeleteOutlined, CheckOutlined } from '@ant-design/icons';
...
const SerialAssignmentModal: React.FC<SerialAssignmentModalProps> = ({
    open, onClose, orderId, detailId, productName, requiredQuantity,
    productDetailId, onSuccess, initialSerials = []
}) => {
    ...
    const fetchAvailableSerials = async () => {
        ...
        try {
            ...
            const merged = [...initialSerials.filter(s => !available.find(a => a.serialNumber === s.serialNumber)),
            ...available];
            setAvailableSerials(merged);
        } catch (error) {
            message.error('Không thể tải danh sách Serial khả dụng');
            ...
            render: (_: any, record: AvailableSerial) => {
                ...
                return (
                    <Button
                        ...
                        onClick={() => alreadyPicked ? handleRemoveSelected(record.serialNumber) : handlePickSerial(record.serialNumber)}
                    >
    ...
    return (
        ...
        >
            <div style={{ marginBottom: 12 }}>
                ...
                <Typography.Text
                    type={selectedSerials.length === requiredQuantity ? 'success' : selectedSerials.length > requiredQuantity ? 'danger' : 'danger'}
                    strong
                >
                    {selectedSerials.length} / {requiredQuantity} Serial
                </Typography.Text>
                {selectedSerials.length === requiredQuantity && (
                    <Tag color="success" style={{ marginLeft: 8 }}>Đã đủ ✓</Tag>
                )}
                {selectedSerials.length > requiredQuantity && (
                    <Tag color="error" style={{ marginLeft: 8 }}>Vượt quá số lượng!</Tag>
                )}
            </div>
            ...
            <Spin spinning={loadingAvailable}>
                <Table
                    ...
                    locale={{ emptyText: 'Không có Serial nào khả dụng cho sản phẩm này' }}
                    rowClassName={(record: AvailableSerial) =>
                        selectedSerials.includes(record.serialNumber) ? 'ant-table-row-selected' : ''
                    }
                />
            </Spin>
            ...
            <Space style={{ marginBottom: 12, width: '100%' }}>
                ...
                <Button icon={<QrcodeOutlined />} onClick={() => setIsScannerOpen(true)}
                    disabled={selectedSerials.length >= requiredQuantity}
                >
                    Quét Camera
                </Button>
