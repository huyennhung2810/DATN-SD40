...
import type { AuthResponse, ChangePasswordRequest, ForgotPasswordRequest, LoginRequest, RefreshTokenRequest, RegisterRequest, VerifyOtpRequest } from "../../models/auth";
...
const authApi = {
    ...
    logout: async (): Promise<void> => {
        await axiosClient.post(AUTH_PATH.LOGOUT);
    },
    ...
    forgotPassword: async (data: ForgotPasswordRequest): Promise<void> => {
        await axiosClient.post(AUTH_PATH.FORGOT_PASSWORD, data);
    },

    //xác thực OTP cho quên mk
    verifyOtp: async (data: VerifyOtpRequest): Promise<void> => {
        await axiosClient.post(AUTH_PATH.VERIFY_OTP, data);
    },
};
